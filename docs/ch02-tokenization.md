...
3. **切换到"真实分词器"**，点击加载 Qwen2.5 模型的真实分词器（约 8MB，首次加载后浏览器会缓存），对比模拟 BPE 和真实分词的差异——你会看到每个 token 的真实 Token ID，这些 ID 就是模型实际使用的编号。
4. **注意统计栏右下角的词表提示**——不同分词方式对词表大小的要求天差地别。

你会发现，"字符级"的 token 数最多但词表最小，"词级"的 token 数最少但词表最大，"BPE 子词"在两者之间取得平衡——这正是BPE类算法被普遍使用的一个原因。

::: details 演示说明
前四种分词方式（字符级、词级、BPE 子词、字节级）使用模拟算法，目的是让你直观理解不同分词方式的差异。"真实分词器"选项使用 Qwen2.5-0.5B-Instruct 模型的实际分词器（基于 BPE），展示真实的 Token ID 和分词结果。不同模型（GPT、Claude、Qwen 等）的词表和合并规则各不相同，但基本原理一致。你也可以使用 OpenAI 的 [Tokenizer](https://platform.openai.com/tokenizer) 在线工具查看 GPT 系列的分词结果。
:::
...
> 在现代 Byte-level BPE 分词器（如 GPT-2+、Qwen）中，由于以字节为底座，理论上不会出现 OOV，因此 `<UNK>` 往往不被使用或仅保留作兼容用途
